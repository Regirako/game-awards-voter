// results.js - Lógica da página de resultados (ATUALIZADA)
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function displayMostVoted(mostVoted) {
        ...
        mostVotedImage.innerHTML = `
            <img src="${imageUrl}" 
                 alt="${gameName}"
                 onerror="this.src='https://via.placeholder.com/380x200/1a5e2c/ffffff?text=${encodeURIComponent(gameName.substring(0, 30))}'">
        `;
    }
    ...
    function getGameImageUrl(gameName) {
        ...
        return `https://via.placeholder.com/380x200/1a5e2c/ffffff?text=${encodeURIComponent(cleanGameName.substring(0, 30))}`;
    }
    
    // Configurar botões de ação
    function setupButtons() {
        // Botão de compartilhar (screenshot simples) - CORRIGIDO
    ...
    // Função para capturar screenshot - SIMPLIFICADA
    function captureScreenshot() {
        ...
        const config = {
            ...
            onclone: function(clonedDoc) {
                // Ajustar o card no clone para screenshot
                const clonedCard = clonedDoc.getElementById('resultCard');
                if (clonedCard) {
                    clonedCard.style.boxShadow = '0 20px 50px rgba(0, 0, 0, 0.5)';
                    clonedCard.style.border = '2px solid var(--gold)';
                }
            }
        };
        
        // Capturar apenas o card
        setTimeout(() => {
            html2canvas(resultCard, config)
                .then(canvas => {
                    ...
                    const fileName = `game-awards-result-${new Date().getTime()}.png`;
                    ...
                    // Mostrar notificação
                    showNotification('Screenshot salvo! Verifique sua pasta de downloads.');
                    ...
                    showNotification('Use a ferramenta de print do seu navegador (Ctrl+P) para salvar como PDF.');
                });
        }, 500);
    }
